export const isInappropriate = (text) => {
  ...
  // Telefon benzeri rakam yoğunluğu - DAHA AKILLI
  const hasPhoneLikeDigits = (s) => {
    // Tüm rakamları çıkar
    const digits = s.replace(/\D/g, '');

    // 1. Direkt telefon formatlarını yakala (boşluk/tire ayrılmış)
    // UK: 07123 456 789, +44 7123 456789
    // TR: 0532 123 4567, +90 532 123 4567
    if (/(?:\+?\d{2,3}[\s\-()]?)?\d{3,4}[\s\-()]+\d{3,4}[\s\-()]+\d{3,4}/.test(s)) return true;

    // 2. Ardışık 9+ rakam (12345678, telefon numarası gibi)
    if (/\d{9,}/.test(s)) return true;

    // 3. Context-aware: "call me", "text me", "phone number" gibi ifadelerden sonra 7+ rakam
    if (/(call|text|phone|whatsapp|wa|contact|reach)[\s\w]*?\d{7,}/.test(s.toLowerCase())) return true;

    // 4. Toplam 11+ rakam VAR ama sadece bunlar telefon patterninde ise flag et
    if (digits.length >= 11) {
      // Eğer rakamlar dağınık değilse (3+ grup halinde), telefon olabilir
      const digitGroups = s.match(/\d+/g) || [];
      const largeGroups = digitGroups.filter(g => g.length >= 3);
      if (largeGroups.length >= 3) return true;
    }
