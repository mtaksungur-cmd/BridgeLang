...
export async function sendMail({ to, subject, html, text }) {
  ...
  // 1. Try SMTP if credentials are present
  if (smtpUser && smtpPass) {
    try {
      return await sendViaSmtp({ to, subject, html: htmlWithFooter, text });
    } catch (smtpErr) {
      console.error('[mailer] SMTP failed, trying Brevo API fallback:', smtpErr.message);
    }
  }

  // 2. Fallback: Brevo HTTP API
  if (brevoApiKey) {
    return await sendViaBrevoApi({ to, subject, html: htmlWithFooter, text });
  }
